	...
	<section class="ftco-section">
	    ...
	    <div class="container">
          ...
          <li class="nav-item"><a href="menu.php" class="nav-link">Menu</a></li>
	        	<li class="nav-item active"><a href="#" class="nav-link">Pesquisas</a></li>
...
switch ($tipo_de_pesquisa){
    ...
    case "genero": 
        $sql_code = "SELECT * FROM cds WHERE genero LIKE '%$pesquisa%' ";
        $result = mysqli_query($conexao,$sql_code);
        while($row = mysqli_fetch_assoc($result)) {
          ...
          echo "</div>";
            ...
            break;
            case "artista": 
                $sql_code = "SELECT * FROM cds WHERE artista LIKE '%$pesquisa%' ";
                $result = mysqli_query($conexao,$sql_code);
                while($row = mysqli_fetch_assoc($result)) {
                  echo "<div style='border-radius:5%;border: 3px solid;margin: 10px 10px; float: left; color:#000; align:center; background-color:#222244';>";
                  $img = $row['img'];
